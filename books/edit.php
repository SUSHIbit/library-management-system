...
$page_title = 'Edit Book: ' . htmlspecialchars($book['title']);
$breadcrumbs = [
...
$errors = [];
$form_data = $book; // Initialize with existing data
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($form_data['isbn'])) {
        $existing = getSingleRow("SELECT id FROM books WHERE isbn = ? AND id != ?", "si", $form_data['isbn'], $book_id);
        if ($existing) $errors[] = 'A book with this ISBN already exists.';
    }

    // Check if quantity reduction affects borrowed books
    $borrowed_count = getSingleRow("SELECT COUNT(*) as count FROM borrowings WHERE book_id = ? AND status = 'borrowed'", "i", $book_id)['count'];
    if ($form_data['quantity'] < $borrowed_count) {
        $errors[] = "Cannot reduce quantity below $borrowed_count (currently borrowed copies).";
    }

    if (empty($errors)) {
        $available_quantity = $form_data['quantity'] - $borrowed_count;
        $publication_year = !empty($form_data['publication_year']) ? $form_data['publication_year'] : null;
        
        $query = "UPDATE books SET title = ?, author = ?, isbn = ?, category_id = ?, quantity = ?, 
                  available_quantity = ?, publication_year = ?, publisher = ?, description = ?, updated_at = NOW() 
                  WHERE id = ?";
        
        $result = executeNonQuery($query, "sssiiiissi",
            $form_data['title'],
            $form_data['author'],
            $form_data['isbn'],
        ...
        if ($result) {
            logActivity('book_updated', "Book updated: {$form_data['title']}", $current_user['id']);
    ...
    <form method="POST" class="book-form" data-validate="true">
        ...
        <div class="form-grid">
            ...
            <div class="form-section">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label for="quantity" class="form-label">Total Quantity <span class="required">*</span></label>
                        <input type="number" id="quantity" name="quantity" class="form-input" 
                               value="<?php echo $form_data['quantity']; ?>" min="1" required>
                        <small class="form-help">
                            Available: <?php echo $book['available_quantity']; ?> | 
                            Borrowed: <?php echo $book['quantity'] - $book['available_quantity']; ?>
                        </small>
                    </div>
                </div>
        ...
        <div class="form-actions">
            <button type="submit" class="btn btn-primary">
                <i class="icon-save"></i> Update Book
            </button>
            <a href="view.php?id=<?php echo $book_id; ?>" class="btn btn-secondary">Cancel</a>
        </div>
    </form>
</div>
